...
public class SticksRenderer extends ShapeRenderer {
  ...
  private void drawBond(int dottedMask) {
    ...
    boolean doFixedSpacing = (bondOrder > 1
        ...
        && (viewer.getHybridizationAndAxes(atomA.index, z, x, "pz") != null || viewer.getHybridizationAndAxes(
            atomB.index, z, x, "pz") != null) && !Float.isNaN(x.x));
    ...
    if (doFixedSpacing) {
      ...
      while (true) {
        ...
        if ((dottedMask & 1) != 0)
          ...
          fillCylinder(colixA, colixB, endcaps, width, s1.x, s1.y, s1.z, s2.x, s2.y, s2.z);
        dottedMask >>= 1;
        if (--bondOrder <= 0)
          break;
        stepAxisCoordinates();
    ...
    while (true) {
      if ((dottedMask & 1) != 0)
        ...
        fillCylinder(colixA, colixB, endcaps, width, xAxis1, yAxis1, zA, xAxis2, yAxis2, zB);
      dottedMask >>= 1;
      if (--bondOrder <= 0)
        break;
      stepAxisCoordinates();
